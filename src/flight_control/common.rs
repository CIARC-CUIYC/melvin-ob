use num_traits::{real::Real, AsPrimitive, Num, NumCast};
use std::ops::{Add, Mul};
    ...
    T: Real + NumCast + std::ops::AddAssign<T>,
{
    ...
    pub fn in_radius_of(&self, other: &Self, rad: T) -> bool {
        if self.euclid_distance(other) <= rad {
            true
        } else {
            false
        }
    ...
    pub fn wrap_around_map(&mut self) {
        ...
        if new_x < T::from(0).unwrap() {
            new_x += Self::map_size().x();
        }
        if new_y < T::from(0).unwrap() {
            new_y += Self::map_size().y();
        }
        self.x = new_x;
        self.y = new_y;
    }
