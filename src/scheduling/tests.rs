...
async fn test_single_target_burn_calculator() {
    ...
    let hit_target =
        est.euclid_distance(res.target_pos()) < detumble_dt * (I32F32::lit("0.002") * detumble_dt);
    if hit_target {
        info!("Test successful. Acc for {acc_dt}s, detumble for {detumble_dt}s!");
    } else {
        info!(
...
async fn test_multi_target_burn_calculator() {
    ...
    if hit_target {
        ...
        info!(
            "Burn Sequence for mocked Zoned Objective at: {target}, due at {mock_end_t}, start at {mock_start_t}"
        );
        log!("Entry at {entry_t}, Position will be {entry_pos}");
        log!("Exit after {acc_dt}s, Position will be {exit_pos}");
